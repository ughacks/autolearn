"""
LSCP Stage 1: Surprisal Detection — DGX Spark Edition
=======================================================
...
def compute_surprisal(model, tokenizer, text):
    ...
    logits = model(tokens).logits[0]  # (seq_len, vocab)
...
def build_gk(surprisals, token_strings, passage):
    ...
    peaks = [
        {"pos": i, "token": token_strings[i], "surprisal": round(float(s[i]), 3)}
    ...
    if third > 0:
        ...
        first_mean = last_mean = drop = d_k = 0.0

    text = passage["text"]
    narrative = {
        "char_len": len(text), "tok_count": len(token_strings),
        "mentions_journal": any(
            j in text for j in
            ["Nature", "Science", "Lancet", "Cell", "PNAS", "JAMA", "BMJ"]
        ),
        "mentions_institution": any(
            u in text for u in
            ["University", "Institute", "MIT", "ETH", "Oxford",
             "Cambridge", "Stanford", "Harvard", "CERN", "NASA", "WHO"]
        ),
    ...
    return {
        ...
        "S_k": round(float(np.mean(s)), 4),
        "d_k": round(d_k, 4),
        "drop": round(drop, 3),
        "first_third": round(first_mean, 3),
        "last_third": round(last_mean, 3),
...
def plot_all(results):
    ...
    colors = {"known": "#4CAF50", "novel": "#FF9800", "corrupt": "#F44336"}
    names = {"known": "Aligned", "novel": "Novel", "corrupt": "Corrupt"}
    lo = ["known", "novel", "corrupt"]
    g = {}
    for r in results:
    ...
    for i, lb in enumerate(lo):
        ...
        if v:
            ax.bar(i, np.mean(v), color=colors[lb], alpha=.7, width=.6,
                   yerr=np.std(v), capsize=5)
            for vv in v:
                ax.scatter(i + np.random.uniform(-.15, .15), vv,
                           color=colors[lb], s=30, zorder=5,
                           edgecolors="white", linewidth=0.5)
    ax.set_xticks(range(3))
    ax.set_xticklabels([names[l] for l in lo])
    ax.set_ylabel("Mean Surprisal")
    ax.set_title(f"Surprisal by Category (n={len(results)})")
    ...
    ax.set_title("High-Surprisal Peaks")
    ax.grid(axis='y', alpha=.3)
    ...
    ax.set_xticks(range(3))
    ax.set_xticklabels([names[l] for l in lo])
    ax.set_ylabel("Drop Ratio (d_k)")
    ax.set_title("In-Context Adaptation")
    ax.grid(axis='y', alpha=.3)
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / "stage1_categories.png", dpi=150)
    plt.close()
    ...
    ax.set_title("All Passages Ranked")
    ax.grid(axis='x', alpha=.3)
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / "stage1_ranked.png", dpi=150)
    plt.close()
...
def main():
    ...
    print(f"  Loaded {len(passages)} passages from {PASSAGE_FILE}")
    ...
    results = []
    short = {"known": "KNOWN ", "novel": "NOVEL ", "corrupt": "CORRPT"}

    print(f"\n{'='*70}")
    print(f"LSCP STAGE 1 [DGX Spark, CUDA] ({len(passages)} passages)")
    print(f"{'='*70}")
    ...
    g = {}
    for r in results:
        g.setdefault(r["label"], []).append(r["S_k"])
    a = np.array(g.get("known", []))
    n = np.array(g.get("novel", []))
    c = np.array(g.get("corrupt", []))
    contra = np.concatenate([n, c]) if len(n) > 0 and len(c) > 0 else np.array([])

    print(f"\n{'='*70}\nANALYSIS\n{'='*70}")
    for lb, v, nm in [("known", a, "ALIGNED"), ("novel", n, "NOVEL"), ("corrupt", c, "CORRUPT")]:
        if len(v) > 0:
            print(f"  {nm:10s}: mean={np.mean(v):.3f}  std={np.std(v):.3f}  "
                  f"min={np.min(v):.3f}  max={np.max(v):.3f}  n={len(v)}")
    ...
    if len(a) > 0:
        ...
        if len(contra) > 0:
            ...
            print(f"\n  Aligned vs Contra: d={d_ac:.3f}  "
                  f"thr={thr:.3f} (mu+{LAMBDA}*sigma)  acc={acc:.1%}")
    if len(n) > 0 and len(c) > 0:
        print(f"  Novel vs Corrupt:  d={cohens_d(n, c):.3f}")

    print(f"\n  Adaptation (mean d_k):")
    for lb in ["known", "novel", "corrupt"]:
        items = [r for r in results if r["label"] == lb]
        if items:
            print(f"    {lb:10s}: {np.mean([r['d_k'] for r in items]):.3f}")
    ...
    output = {
        "model": MODEL_NAME, "backend": "PyTorch + bitsandbytes (DGX Spark)",
        "n_passages": len(results), "results": results, "analysis": analysis,
    }
